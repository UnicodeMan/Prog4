...
int main()
{
	...
	// Užduočių skaičius (n), skaitikliai ir vardikliai 1-2, suma, skirtumas, --
	// -- sandauga, dalmuo
...
void trupm_veiksmai(int skaitl1, int vardl1, int skaitl2, int vardl2, int &suma1, int &suma2, int &skirt1, int &skirt2, int &sand1, int &sand2, int &dalm1, int &dalm2){
    int i=0, j=0, ats1, ats2, vardl_common;
    ...
    // ats yra
    // skaičiai --> ats1, ats2
    for (j; j < 6 ;j++) { // cikle esančiuose komentaruose naudotos komandos buvo skirtos klaidų radimui
        if (j = 1) {
            ats1 = suma1;
            ats2 = suma2;
            for (i=0; i < 2; i++) {
